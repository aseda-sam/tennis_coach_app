#!/usr/bin/env python3
"""
Performance testing script for video processing limits.
Tests different video configurations to determine optimal defaults.
"""
...
class PerformanceTester:
    """Test video processing performance across different configurations."""
    ...
    def run_comprehensive_test(self, test_dir: Path = Path("test_videos")) -> None:
        ...
        for resolution, fps, duration, description in test_configs:
            ...
            # Extract frames for processing tests
            frames = []
            cap = cv2.VideoCapture(str(video_path))
            while True:
                ret, frame = cap.read()
    ...
    def generate_recommendations(self) -> Dict[str, any]:
        ...
        # Analyze 4K performance
        fourk_results = [r for r in self.results if r["resolution"] == (3840, 2160)]
        if fourk_results:
            ...
            if avg_yolo_fps > 5:  # If we can process 4K at >5fps
                recommendations["max_resolution"] = (3840, 2160)
                recommendations["reasoning"].append("4K processing is viable")
            else:
                recommendations["reasoning"].append(
                    "4K processing too slow, limiting to 1080p"
                )
        ...
        if high_fps_results:
            ...
            if avg_yolo_fps < 10:  # If 60fps processing is slow
                recommendations["reasoning"].append(
