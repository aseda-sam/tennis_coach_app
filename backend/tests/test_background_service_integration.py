    ...
    def test_pose_only_analysis_routing(self, db_session: Session) -> None:
        ...
        with patch("app.services.background_service.PoseDetectionService") as mock_pose_service, \
             ...
             patch("pathlib.Path.exists", return_value=True):
                            ...
                            mock_pose_instance.save_detection_results.return_value = (
                                Mock(id=1)
                            )
    ...
    def test_ball_only_analysis_routing(self, db_session: Session) -> None:
        ...
        with patch("app.services.background_service.BallDetectionService") as mock_ball_service, \
             ...
             patch("pathlib.Path.exists", return_value=True):
                            ...
                            mock_ball_instance.save_detection_results.return_value = (
                                Mock(id=1)
                            )
    ...
    def test_comprehensive_analysis_routing(self, db_session: Session) -> None:
        ...
        with patch("app.services.background_service.PoseDetectionService") as mock_pose_service, \
             ...
             patch("pathlib.Path.exists", return_value=True):
                                ...
                                mock_pose_instance.save_detection_results.return_value = Mock(
                                    id=1
                                )
                                mock_ball_instance.save_detection_results.return_value = Mock(
                                    id=2
                                )
