...
class BallDetectionMetrics(BaseModel):
    ...
    frames_with_balls: int = Field(..., description="Number of frames containing ball detections")
    total_detections: int = Field(..., description="Total number of ball detections")
    detection_rate: float = Field(..., description="Percentage of frames with ball detections (0.0-1.0)")
    average_detections_per_frame: float = Field(..., description="Average number of ball detections per frame")

    # Model information
    model_used: str = Field(..., description="YOLO model used for detection (e.g., 'yolov8n')")
    confidence_threshold: float = Field(..., description="Confidence threshold used for detections")
    model_selection_reason: Optional[str] = Field(None, description="Reason for model selection")

    # Quality metrics
    average_confidence: Optional[float] = Field(None, description="Average confidence score of detections")
    min_confidence: Optional[float] = Field(None, description="Minimum confidence score")
    max_confidence: Optional[float] = Field(None, description="Maximum confidence score")

    # Performance metrics
    processing_time_seconds: float = Field(..., description="Total processing time in seconds")
    frame_processing_rate: Optional[float] = Field(None, description="Frames processed per second")
...
class BallDetectionResponse(BaseModel):
    ...
    status: str = Field(..., description="Detection status (completed, failed, processing)")
    ...
    completed_at: Optional[datetime] = Field(None, description="When detection was completed")
