...
- **Stored** — Pose data → `pose_detections` table (detection_mode: scout or refine); serve windows/proposals → `serve_windows` table (one row per window; `status` tracks pending/accepted/rejected/edited).
